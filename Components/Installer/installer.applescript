...
on synergyInstall(inDomain)
	...
	tell application "System Events"
		set folderpath to POSIX path of (path to me)
	end tell
	set helpBookPath to quoted form of (folderpath & "/Contents/Resources/SynergyKM Help")
	set prefpanePath to quoted form of (folderpath & "/Contents/Resources/SynergyKM.prefPane")
	...
	if inDomain is "User" then
		...
		try
			...
			do shell script "xattr -d com.apple.quarantine /Library/PreferencePanes/SynergyKM.prefPane/Contents/Resources/Synergyd.app || true" with administrator privileges
			do shell script "xattr -d com.apple.quarantine /Library/PreferencePanes/SynergyKM.prefPane/Contents/Resources/Synergyd.app/Contents/MacOS/Synergyd || true" with administrator privileges
			set r to true
		end try
	end if
	return r
