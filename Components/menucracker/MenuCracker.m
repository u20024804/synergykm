...
{
    if (!(*_originalLoadClass)(self, _cmd, className)) {
...
static void freeTheUserToLoadAnyMenulingTheyWant()
{
    ...
    _originalCreateMenuExtra = method_getImplementation(method);
    method_setImplementation(method, (IMP)fixUpMenuExtraLoad);
    ...
    _originalLoadClass = method_getImplementation(method);
    method_setImplementation(method, (IMP)justSayYESIWantToLoadClass);
	...
	_originalWriteDefaults = method_getImplementation(method);
    method_setImplementation(method, (IMP)hackWriteAsWellWhileWeAreAtIt);
